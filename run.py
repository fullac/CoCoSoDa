        ...
        if os.path.exists(cache_file):
            ...
            if self.args.data_aug_type == "replace_type":
                self.examples = pool.map(
                    convert_examples_to_features_aug_type, tqdm(data, total=len(data))
                )
            else:
                self.examples = pool.map(
...
def convert_examples_to_features_unixcoder(js, tokenizer, args):
    ...
    code = (
        " ".join(js["code_tokens"])
        if type(js["code_tokens"]) is list
    ...
    nl = (
        ...
        if type(js["docstring_tokens"]) is list
...
def mask_tokens(inputs, tokenizer, mlm_probability):
    ...
    probability_matrix.masked_fill_(
        torch.tensor(special_tokens_mask, dtype=torch.bool).to(inputs.device), value=0.0
    )
    ...
    masked_indices = torch.bernoulli(
        probability_matrix
    ).bool()  # will decide who will be masked
...
def replace_special_token_with_type_tokens(
    ...
    probability_matrix.masked_fill_(
        labels.eq(speical_token_ids).to(inputs.device), value=mlm_probability
    )
    masked_indices = torch.bernoulli(
        probability_matrix
    ).bool()  # will decide who will be masked
...
def replace_special_token_with_mask(
    inputs, speical_token_ids, tokenizer, mlm_probability
):
    """Prepare masked tokens inputs/labels for masked language modeling: 80% MASK, 10% random, 10% original."""
    labels = inputs.clone()
...
def train(args, model, tokenizer, pool):
    ...
    if args.data_aug_type == "replace_type":
        ...
        train_dataset = TextDataset_unixcoder(
            tokenizer, args, args.train_data_file, pool
        )
        # else:
        #     train_dataset=TextDataset(tokenizer, args, args.train_data_file, pool)
    ...
    logger.info(
        "  Instantaneous batch size per GPU = %d", args.train_batch_size // args.n_gpu
    )
    logger.info("  Total train batch size  = %d", args.train_batch_size)
    logger.info(
        "  Total optimization steps = %d", len(train_dataloader) * args.num_train_epochs
    )
    ...
    for idx in range(args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            ...
            loss = loss_fct(
                scores * 20, torch.arange(code_inputs.size(0), device=scores.device)
            )
        ...
        results = evaluate(
            args, model, tokenizer, args.eval_data_file, pool, eval_when_training=True
        )
...
def multi_lang_continue_pre_train(args, model, tokenizer, pool):
    ...
    if "unixcoder" in args.model_name_or_path:
        ...
        for train_data_file in args.couninue_pre_train_data_files:
            train_dataset = TextDataset_unixcoder(
                tokenizer, args, train_data_file, pool
            )
    ...
    while True:
        ...
        if step % 4 == 0:
            ...
            code_transformations_ids[:, 3:], _ = replace_special_token_with_type_tokens(
                code_inputs.clone()[:, 3:],
                choice_token_id,
                tokenizer,
                args.mlm_probability,
            ...
            random.seed(step)
            choice_token_id = choice(special_token_id_list)
            code_transformations_ids[:, 3:], _ = replace_special_token_with_mask(
                code_inputs.clone()[:, 3:],
                choice_token_id,
                tokenizer,
                args.mlm_probability,
        ...
        if (step + 1) % args.gradient_accumulation_steps == 0:
            ...
            if (
                ...
                output_dir = os.path.join(
                    args.output_dir,
                    "{}-{}-{}".format(
                        checkpoint_prefix, global_step, round(results["eval_mrr"], 6)
                    ),
                )
                if not os.path.exists(output_dir):
                    os.makedirs(output_dir)
...
def evaluate(args, model, tokenizer, file_name, pool, eval_when_training=False):
            ...
            if args.model_type == "base":
                ...
                if args.agg_way == "avg":
                    ...
                    nl_vec = (outputs * nl_inputs.ne(1)[:, :, None]).sum(
                        1
                    ) / nl_inputs.ne(1).sum(-1)[
                        :, None
                    ]  # None作为ndarray或tensor的索引作用是增加维度，
    ...
    if args.save_evaluation_reuslt:
        ...
        save_pickle_data(
            args.save_evaluation_reuslt_dir, "evaluation_result.pkl", evaluation_result
        )
...
def parse_args():
    ...
    parser.add_argument(
        "--epoch", type=int, default=50, help="random seed for initialization"
    )
    ...
    parser.add_argument(
        "--do_fine_tune", action="store_true", help="debug mode", required=False
    )
    parser.add_argument(
        "--do_whitening",
        action="store_true",
    ...
    parser.add_argument(
        "--time_score", default=1, type=int, help="cosine value * time_score"
    )
    parser.add_argument(
        "--max_steps",
        default=100,
    ...
    # debug
    parser.add_argument(
        "--use_best_mrr_model", action="store_true", help="cosine_space", required=False
    )
    parser.add_argument(
        "--debug", action="store_true", help="debug mode", required=False
    )
    ...
    # model type
    parser.add_argument(
        "--do_avg", action="store_true", help="avrage hidden status", required=False
    )
    parser.add_argument(
        "--model_type",
        default="base",
    ...
    parser.add_argument(
        "--moco_dim", default=768, type=int, help="feature dimension (default: 768)"
    )
    parser.add_argument(
        "--moco_k",
        default=32,
...
def create_model(args, model, tokenizer, config=None):
    ...
    if (
        args.data_aug_type in ["replace_type", "other"]
    ...
    if (args.loaded_model_filename) and (
        "pytorch_model.bin" in args.loaded_model_filename
    ):
    ...
    if (args.loaded_model_filename) and (
        "pytorch_model.bin" not in args.loaded_model_filename
    ):
    ...
    if args.loaded_codebert_model_filename:
        logger.info(
            "reload pytorch model from {}".format(args.loaded_codebert_model_filename)
        )
        model.load_state_dict(
            torch.load(args.loaded_codebert_model_filename), strict=False
        )
