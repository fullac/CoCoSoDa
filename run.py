...
def cal_r1_r5_r10(ranks):
    ...
    for item in ranks:
        if item >= 1:
            ...
            r10 += 1
    result = {
        "R@1": round(r1 / data_len, 3),
        "R@5": round(r5 / data_len, 3),
        "R@10": round(r10 / data_len, 3),
    }
    return result
...
def convert_examples_to_features(js):
    ...
    # code
    code_tokens = tokenizer_source_code(js["original_string"], parser,
                                        args.lang)
...
def convert_examples_to_features_aug_type(js):
    ...
    code_type_tokens = (
        [tokenizer.cls_token, "<encoder-only>", tokenizer.sep_token] +
        code_type_tokens + [tokenizer.sep_token])
    ...
    nl_tokens = ([tokenizer.cls_token, "<encoder-only>", tokenizer.sep_token] +
                 nl_tokens + [tokenizer.sep_token])
    nl_ids = tokenizer.convert_tokens_to_ids(nl_tokens)
    padding_length = args.nl_length - len(nl_ids)
    nl_ids += [tokenizer.pad_token_id] * padding_length

    return TypeAugInputFeatures(
        code_tokens,
        code_ids,
        code_type_tokens,
        code_type_ids,
    ...
    def __init__(self, tokenizer, args, file_path=None, pool=None):
        ...
        if os.path.exists(cache_file):
            ...
            if self.args.data_aug_type == "replace_type":
                self.examples = pool.map(convert_examples_to_features_aug_type,
                                         tqdm(data, total=len(data)))
            else:
                self.examples = pool.map(convert_examples_to_features,
                                         tqdm(data, total=len(data)))
        ...
        if "train" in file_path:
            for idx, example in enumerate(self.examples[:3]):
                ...
                logger.info("code_ids: {}".format(" ".join(
                    map(str, example.code_ids))))
                logger.info("nl_tokens: {}".format(
                    [x.replace("\u0120", "_") for x in example.nl_tokens]))
                logger.info("nl_ids: {}".format(" ".join(
                    map(str, example.nl_ids))))
...
def convert_examples_to_features_unixcoder(js, tokenizer, args):
    ...
    code = (" ".join(js["code_tokens"]) if type(js["code_tokens"]) is list else
    ...
    nl = (" ".join(js["docstring_tokens"]) if type(js["docstring_tokens"])
          is list else " ".join(js["doc"].split()))
    nl_tokens = tokenizer.tokenize(nl)[:args.nl_length - 4]
    nl_tokens = ([tokenizer.cls_token, "<encoder-only>", tokenizer.sep_token] +
                 nl_tokens + [tokenizer.sep_token])
    nl_ids = tokenizer.convert_tokens_to_ids(nl_tokens)
    ...
    def __init__(self, tokenizer, args, file_path=None, pooler=None):
        ...
        for js in data:
            self.examples.append(
                convert_examples_to_features_unixcoder(js, tokenizer, args))
...
def mask_tokens(inputs, tokenizer, mlm_probability):
    ...
    probability_matrix = torch.full(labels.shape,
                                    mlm_probability).to(inputs.device)
    special_tokens_mask = [
        tokenizer.get_special_tokens_mask(val, already_has_special_tokens=True)
        for val in labels.tolist()
    ]  # for masking special token
    ...
    if tokenizer._pad_token is not None:
        ...
        probability_matrix.masked_fill_(padding_mask,
                                        value=0.0)  # masked padding

    masked_indices = torch.bernoulli(
        probability_matrix).bool()  # will decide who will be masked
    labels[~masked_indices] = -100  # We only compute loss on masked tokens

    # 80% of the time, we replace masked input tokens with tokenizer.mask_token ([MASK])
    indices_replaced = (
        torch.bernoulli(torch.full(labels.shape, 0.8)).bool().to(inputs.device)
        & masked_indices)
    ...
    indices_random = (
        torch.bernoulli(torch.full(labels.shape, 0.5)).bool().to(inputs.device)
        & masked_indices
        & ~indices_replaced)
...
def replace_special_token_with_type_tokens(inputs, speical_token_ids,
                                           tokenizer, mlm_probability):
    """Prepare masked tokens inputs/labels for masked language modeling: 80% MASK, 10% random, 10% original."""
    labels = inputs.clone()
    probability_matrix = torch.full(labels.shape, 0.0).to(inputs.device)
    probability_matrix.masked_fill_(labels.eq(speical_token_ids).to(
        inputs.device),
...
def replace_special_token_with_mask(inputs, speical_token_ids, tokenizer,
                                    mlm_probability):
    """Prepare masked tokens inputs/labels for masked language modeling: 80% MASK, 10% random, 10% original."""
    labels = inputs.clone()
    probability_matrix = torch.full(labels.shape, 0.0).to(inputs.device)
...
def train(args, model, tokenizer, pool):
    ...
    if args.data_aug_type == "replace_type":
        ...
        train_dataset = TextDataset_unixcoder(tokenizer, args,
                                              args.train_data_file, pool)
        # else:
    ...
    for idx in range(args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            ...
            loss = loss_fct(
                scores * 20,
                torch.arange(code_inputs.size(0), device=scores.device))
...
def multi_lang_continue_pre_train(args, model, tokenizer, pool):
    ...
    train_samplers = [
        RandomSampler(train_dataset) for train_dataset in train_datasets
    ]
    ...
    while True:
        ...
        if step % 4 == 0:
            code_transformations_ids[:, 3:], _ = mask_tokens(
                code_inputs.clone()[:, 3:], tokenizer, args.mlm_probability)
            nl_transformations_ids[:, 3:], _ = mask_tokens(
                nl_inputs.clone()[:, 3:], tokenizer, args.mlm_probability)
        elif step % 4 == 1:
            code_types = code_inputs.clone()
            code_transformations_ids[:, 3:], _ = replace_with_type_tokens(
                code_inputs.clone()[:, 3:],
                code_types.clone()[:, 3:],
                tokenizer,
            ...
            code_transformations_ids[:,
                                     3:], _ = replace_special_token_with_type_tokens(
                                         code_inputs.clone()[:, 3:],
                                         choice_token_id,
                                         tokenizer,
                                         args.mlm_probability,
            ...
            choice_token_id = choice(special_token_id_list)
            code_transformations_ids[:,
                                     3:], _ = replace_special_token_with_mask(
                                         code_inputs.clone()[:, 3:],
                                         choice_token_id,
                                         tokenizer,
                                         args.mlm_probability,
        ...
        tr_num += 1
        inter_output, inter_target, _, _ = model(
            source_code_q=code_inputs,
            source_code_k=code_transformations_ids,
            nl_q=nl_inputs,
            nl_k=nl_transformations_ids,
        )
        ...
        tr_loss += loss.item()
        if (step + 1) % args.eval_frequency == 0:
            logger.info("step {} loss {}".format(step + 1,
        ...
        if (step + 1) % args.gradient_accumulation_steps == 0:
            ...
            global_step += 1
            output_flag = True
            avg_loss = round((tr_loss - logging_loss) / (global_step - tr_nb),
                             6)

            if global_step % 100 == 0:
                logger.info(
                    " global steps (step*gradient_accumulation_steps ): %s loss: %s",
                    global_step,
                    round(avg_loss, 6),
                )
            ...
            if (args.local_rank in [-1, 0] and args.save_steps > 0
                ...
                logger.info("  %s = %s", "eval_mrr",
                            round(results["eval_mrr"], 6))
                ...
                if results["eval_mrr"] > best_mrr:
                    ...
                    output_dir = os.path.join(
                        args.output_dir, "{}".format("checkpoint-best-mrr"))
                    if not os.path.exists(output_dir):
                        os.makedirs(output_dir)
                ...
                if not os.path.exists(output_dir):
                    os.makedirs(output_dir)
                model_to_save = (
                    model.module.code_encoder_q
                    if hasattr(model, "module") else model.code_encoder_q
...
def evaluate(args,
    ...
    for batch in query_dataloader:
        ...
        with torch.no_grad():
            if args.model_type == "base":
                ...
                outputs = model_eval.nl_encoder_q(
                    nl_inputs, attention_mask=nl_inputs.ne(1))
                if args.agg_way == "avg":
                    outputs = outputs[0]
                    nl_vec = (outputs * nl_inputs.ne(1)[:, :, None]).sum(
                        1) / nl_inputs.ne(1).sum(
                            -1)[:, None]  # None作为ndarray或tensor的索引作用是增加维度，
                elif args.agg_way == "cls_pooler":
                    nl_vec = outputs[1]
                elif args.agg_way == "avg_cls_pooler":
                    nl_vec = (
                        outputs[1] +
                        (outputs[0] * nl_inputs.ne(1)[:, :, None]).sum(1) /
    ...
    for batch in code_dataloader:
        with torch.no_grad():
            ...
            if args.model_type == "base":
                ...
                outputs = model_eval.code_encoder_q(
                    code_inputs, attention_mask=code_inputs.ne(1))
                if args.agg_way == "avg":
                    outputs = outputs[0]
                    code_vec = (outputs * code_inputs.ne(1)[:, :, None]).sum(
                        1) / code_inputs.ne(1).sum(
                            -1)[:, None]  # None作为ndarray或tensor的索引作用是增加维度，
                elif args.agg_way == "cls_pooler":
                    code_vec = outputs[1]
                elif args.agg_way == "avg_cls_pooler":
                    code_vec = (
                        outputs[1] +
                        (outputs[0] * code_inputs.ne(1)[:, :, None]).sum(1) /
    ...
    sort_ids = np.argsort(scores, axis=-1, kind="quicksort",
                          order=None)[:, ::-1]
    ...
    if args.save_evaluation_reuslt:
        evaluation_result = {
            "nl_urls": nl_urls,
            "code_urls": code_urls,
            "sort_ids": sort_ids[:, :10],
            "ranks": ranks,
        }
        save_pickle_data(args.save_evaluation_reuslt_dir,
                         "evaluation_result.pkl", evaluation_result)
...
def parse_args():
    ...
    parser.add_argument(
        "--data_aug_type",
        default="replace_type",
        choices=["replace_type", "random_mask", "other"],
        help="the ways of soda",
        required=False,
    )
    parser.add_argument(
        "--aug_type_way",
        default="random_replace_type",
        choices=[
            "random_replace_type",
            "replace_special_type",
            "replace_special_type_with_mask",
        ],
        help="the ways of soda",
        required=False,
    )
    parser.add_argument(
        "--print_align_unif_loss",
        action="store_true",
        help="print_align_unif_loss",
        required=False,
    )
    parser.add_argument(
        "--do_ineer_loss",
    ...
    parser.add_argument(
        "--agg_way",
        default="avg",
        choices=["avg", "cls_pooler", "avg_cls_pooler"],
        help="base is codebert/graphcoder/unixcoder",
        required=False,
    )
    parser.add_argument("--weight_decay",
                        ...
                        required=False)
    parser.add_argument(
        "--do_single_lang_continue_pre_train",
        action="store_true",
        help="do_single_lang_continue_pre_train",
        required=False,
    )
    parser.add_argument(
        "--save_evaluation_reuslt",
        action="store_true",
        help="save_evaluation_reuslt",
        required=False,
    )
    parser.add_argument(
        "--save_evaluation_reuslt_dir",
        type=str,
        help="save_evaluation_reuslt",
        required=False,
    )
    ...
    parser.add_argument(
        "--loaded_codebert_model_filename",
        type=str,
        required=False,
        help="loaded_model_filename",
    )
    parser.add_argument(
        ...
        action="store_true",
        help="do_multi_lang_continue_pre_train",
        required=False,
    )
    parser.add_argument(
        "--couninue_pre_train_data_files",
        default=[
    ...
    #                     help="The input training data files (some json files).")
    parser.add_argument(
        "--do_continue_pre_trained",
        action="store_true",
        help="debug mode",
        required=False,
    )
    parser.add_argument("--do_fine_tune",
                        action="store_true",
                        help="debug mode",
                        required=False)
    parser.add_argument(
        "--do_whitening",
        action="store_true",
        help="do_whitening https://github.com/Jun-jie-Huang/WhiteningBERT",
        required=False,
    )
    parser.add_argument("--time_score",
                        default=1,
                        type=int,
                        help="cosine value * time_score")
    parser.add_argument(
        "--max_steps",
        default=100,
        type=int,
    ...
    # debug
    parser.add_argument("--use_best_mrr_model",
                        action="store_true",
                        help="cosine_space",
                        required=False)
    parser.add_argument("--debug",
                        ...
                        help="debug mode",
                        required=False)
    parser.add_argument("--n_debug_samples",
                        type=int,
                        default=100,
                        required=False)
    parser.add_argument(
    ...
    # model type
    parser.add_argument("--do_avg",
                        action="store_true",
                        help="avrage hidden status",
                        required=False)
    parser.add_argument(
        "--model_type",
        default="base",
    ...
    parser.add_argument(
        "--train_data_file",
        default="dataset/java/train.jsonl",
        type=str,
        required=False,
    ...
    parser.add_argument(
        "--output_dir",
        default="saved_models/pre-train",
        type=str,
        required=False,
    ...
    parser.add_argument(
        "--eval_data_file",
        default="dataset/java/valid.jsonl",
        type=str,
        help="An optional input evaluation data file to evaluate the MRR(a jsonl file).",
    )
    parser.add_argument(
        "--test_data_file",
        default="dataset/java/test.jsonl",
        type=str,
        help="An optional input test data file to test the MRR(a josnl file).",
    )
    parser.add_argument(
        "--codebase_file",
        default="dataset/java/codebase.jsonl",
        type=str,
...
def create_model(args, model, tokenizer, config=None):
    ...
    if (args.data_aug_type in ["replace_type", "other"]
            and not args.only_save_the_nl_code_vec):
    ...
    if (args.loaded_model_filename) and ("pytorch_model.bin"
                                         in args.loaded_model_filename):
        logger.info("reload pytorch model from {}".format(
            args.loaded_model_filename))
    ...
    if (args.loaded_model_filename) and ("pytorch_model.bin"
                                         not in args.loaded_model_filename):
    ...
    if args.loaded_codebert_model_filename:
        logger.info("reload pytorch model from {}".format(
            args.loaded_codebert_model_filename))
