...
def DFG_python(root_node, index_to_code, states):
    ...
    if (len(root_node.children) == 0
            or root_node.type == "string") and root_node.type != "comment":
        idx, code = index_to_code[(root_node.start_point, root_node.end_point)]
        if root_node.type == code:
            return [], states
        elif code in states:
            return [(code, idx, "comesFrom", [code], states[code].copy())
                    ], states
        else:
        ...
        if root_node.type == "for_in_clause":
            ...
            if root_node.child_by_field_name("right") is None:
                return [], states
            left_nodes = [
                x for x in root_node.child_by_field_name("left").children
                if x.type != ","
            ]
            right_nodes = [
                x for x in root_node.child_by_field_name("right").children
                if x.type != ","
            ]
        ...
        for left_node, right_node in zip(left_nodes, right_nodes):
            left_tokens_index = tree_to_variable_index(left_node,
                                                       index_to_code)
        ...
        for child in root_node.children:
            ...
            if child.type not in ["elif_clause", "else_clause"]:
                temp, current_states = DFG_python(child, index_to_code,
                                                  current_states)
                DFG += temp
            else:
        ...
        for i in range(2):
            ...
            if root_node.children[-1].type == "block":
                temp, states = DFG_python(root_node.children[-1],
        ...
        for x in DFG:
            if (x[0], x[1], x[2]) not in dic:
                ...
                dic[(x[0], x[1],
                     x[2])][0] = list(set(dic[(x[0], x[1], x[2])][0] + x[3]))
    ...
    if (len(root_node.children) == 0
        ...
        for child in root_node.children:
            ...
            if child.type not in if_statement and flag is False:
                temp, current_states = DFG_java(child, index_to_code,
                                                current_states)
                DFG += temp
            else:
                flag = True
    ...
    if (len(root_node.children) == 0
        ...
        for child in root_node.children:
            if "else" in child.type:
                tag = True
            if child.type not in if_statement and flag is False:
                temp, current_states = DFG_csharp(child, index_to_code,
                                                  current_states)
                DFG += temp
            else:
                flag = True
    ...
    if (len(root_node.children) == 0
        ...
        for child in root_node.children:
            ...
            if child.type not in if_statement:
                temp, current_states = DFG_ruby(child, index_to_code,
                                                current_states)
                DFG += temp
            else:
        ...
        for i in range(2):
            ...
            for left_node, right_node in zip(left_nodes, right_nodes):
                ...
                for token1_index in left_tokens_index:
                    ...
                    states[code1] = [idx1]
                DFG += temp
            temp, states = DFG_ruby(root_node.child_by_field_name("body"),
                                    index_to_code, states)
            DFG += temp
        dic = {}
        for x in DFG:
    ...
    if (len(root_node.children) == 0
        ...
        for child in root_node.children:
            if "else" in child.type:
                tag = True
            if child.type not in if_statement and flag is False:
                temp, current_states = DFG_go(child, index_to_code,
                                              current_states)
                DFG += temp
            else:
                flag = True
        ...
        for child in root_node.children:
            if flag:
                ...
                if child.child_by_field_name("update") is not None:
                    temp, states = DFG_go(child.child_by_field_name("update"),
                                          index_to_code, states)
                    DFG += temp
                flag = True
        dic = {}
    ...
    if (len(root_node.children) == 0
        ...
        for child in root_node.children:
            if "else" in child.type:
                tag = True
            if child.type not in if_statement and flag is False:
                temp, current_states = DFG_php(child, index_to_code,
                                               current_states)
                DFG += temp
            else:
                flag = True
    ...
    if (len(root_node.children) == 0
        ...
        for child in root_node.children:
            if "else" in child.type:
                tag = True
            if child.type not in if_statement and flag is False:
                temp, current_states = DFG_javascript(child, index_to_code,
                                                      current_states)
                DFG += temp
            else:
                flag = True
