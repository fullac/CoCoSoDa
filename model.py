...
class Multi_Loss_CoCoSoDa(BaseModel):
    ...
    def __init__(self, base_encoder, args, mlp=False):
        ...
        for param_q, param_k in zip(
            self.code_encoder_q.parameters(), self.code_encoder_k.parameters()
        ):
            param_k.data.copy_(param_q.data)  # initialize
            param_k.requires_grad = False  # not update by gradient

        for param_q, param_k in zip(
            self.nl_encoder_q.parameters(), self.nl_encoder_k.parameters()
        ):
            param_k.data.copy_(param_q.data)  # initialize
            param_k.requires_grad = False  # not update by gradient
    ...
    def _momentum_update_key_encoder(self):
        ...
        if self.mlp:
            for param_q, param_k in zip(
                self.code_encoder_q_fc.parameters(), self.code_encoder_k_fc.parameters()
            ):
                param_k.data = param_k.data * self.m + param_q.data * (1.0 - self.m)
            for param_q, param_k in zip(
                self.nl_encoder_q_fc.parameters(), self.nl_encoder_k_fc.parameters()
            ):
                param_k.data = param_k.data * self.m + param_q.data * (1.0 - self.m)
    ...
    def forward(self, source_code_q, source_code_k, nl_q, nl_k):
        ...
        if not self.args.do_multi_lang_continue_pre_train:
            ...
            outputs = self.code_encoder_q(
                source_code_q, attention_mask=source_code_q.ne(1)
            )[0]
            code_q = (outputs * source_code_q.ne(1)[:, :, None]).sum(
                1
            ) / source_code_q.ne(1).sum(-1)[
                :, None
            ]  # None作为ndarray或tensor的索引作用是增加维度，
            ...
            outputs = self.nl_encoder_q(nl_q, attention_mask=nl_q.ne(1))[
                0
            ]  # queries: NxC   bs*feature_dim
            nl_q = (outputs * nl_q.ne(1)[:, :, None]).sum(1) / nl_q.ne(1).sum(-1)[
                :, None
            ]
            ...
            # label
            code2nl_label = torch.arange(
                code2nl_logits.size(0), device=code2nl_logits.device
            )
        ...
        if self.agg_way == "avg":
            ...
            with torch.no_grad():  # no gradient to keys
                ...
                code_k = (outputs * source_code_k.ne(1)[:, :, None]).sum(
                    1
                ) / source_code_k.ne(1).sum(-1)[
                    :, None
                ]  # None作为ndarray或tensor的索引作用是增加维度，
                ...
                nl_k = (outputs * nl_k.ne(1)[:, :, None]).sum(1) / nl_k.ne(1).sum(-1)[
                    :, None
                ]
            ...
            outputs = outputs[0]
            code_q_avg = (outputs * source_code_q.ne(1)[:, :, None]).sum(
                1
            ) / source_code_q.ne(1).sum(-1)[
                :, None
            ]  # None作为ndarray或tensor的索引作用是增加维度，
            ...
            nl_q_avg = (outputs * nl_q.ne(1)[:, :, None]).sum(1) / nl_q.ne(1).sum(-1)[
                :, None
            ]
            ...
            with torch.no_grad():  # no gradient to keys
                ...
                outputs = outputs[0]
                nl_k_avg = (outputs * nl_k.ne(1)[:, :, None]).sum(1) / nl_k.ne(1).sum(
                    -1
                )[:, None]
        ...
        code2maskednl_neg = torch.einsum(
            "nc,ck->nk", [code_q, self.masked_nl_queue.clone().detach()]
        )
        # logits: Nx(n+K)
        ...
        # label
        code2maskednl_label = torch.arange(
            code2maskednl_logits.size(0), device=code2maskednl_logits.device
        )
        ...
        # label
        nl2code_label = torch.arange(
            nl2code_logits.size(0), device=nl2code_logits.device
        )
        ...
        # label
        nl2maskedcode_label = torch.arange(
            nl2maskedcode_logits.size(0), device=nl2maskedcode_logits.device
        )
        ...
        inter_labels = torch.cat(
            (code2nl_label, code2maskednl_label, nl2code_label, nl2maskedcode_label),
            dim=0,
        )
        ...
        if self.do_ineer_loss:
            ...
            code2maskedcode_neg = torch.einsum(
                "nc,ck->nk", [code_q, self.masked_code_queue.clone().detach()]
            )
            # logits: Nx(n+K)
            code2maskedcode_logits = torch.cat(
                [self.time_score * code2maskedcode_pos, code2maskedcode_neg], dim=1
            )
            # apply temperature
            code2maskedcode_logits /= self.T
            # label
            code2maskedcode_label = torch.arange(
                code2maskedcode_logits.size(0), device=code2maskedcode_logits.device
            )
            ...
            nl2maskednl_neg = torch.einsum(
                "nc,ck->nk", [nl_q, self.masked_nl_queue.clone().detach()]
            )
            # nl2code_logits: bsx(n+K)
            ...
            # label
            nl2maskednl_label = torch.arange(
                nl2maskednl_logits.size(0), device=nl2maskednl_logits.device
            )

            # logit 6*bsx(1+K)
            inter_logits = torch.cat(
                (inter_logits, code2maskedcode_logits, nl2maskednl_logits), dim=0
            )

            # labels: positive key indicators
            # inter_labels = torch.zeros(inter_logits.shape[0], dtype=torch.long).cuda()
            inter_labels = torch.cat(
                (inter_labels, code2maskedcode_label, nl2maskednl_label), dim=0
            )
